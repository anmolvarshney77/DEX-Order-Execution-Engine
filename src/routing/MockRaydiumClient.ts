...
/**
 * Mock implementation of Raydium DEX client
 * Simulates realistic delays and price variance for testing
 */
export class MockRaydiumClient implements DexClient {
  ...
  private readonly fee: number = 0.0025; // 0.25% fee

  /**
   * Get a simulated price quote from Raydium
  ...
  async getQuote(tokenIn: string, tokenOut: string, amount: number): Promise<DexQuote> {
    ...
    return {
      ...
      fee: this.fee,
      effectivePrice,
      poolId: `raydium-pool-${tokenIn.slice(0, 8)}-${tokenOut.slice(0, 8)}`,
      estimatedOutput
    };
  }

  /**
   * Execute a simulated swap on Raydium
  ...
  async executeSwap(params: SwapParams): Promise<SwapResult> {
    ...
    return {
      txHash: generateMockTxHash('raydium'),
      executedPrice,
